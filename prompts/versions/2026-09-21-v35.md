# Current prompt policy: 2026-09-21-v35

Behavior release: `digital-equity-conversation-grounding`. Display version: **v1.35**.

## Change log

Keep the latest five complete exchanges verbatim for conversational continuity while limiting stale context. Preserve the named program during signup follow-ups, distinguish booking-widget availability from calendar sessions, keep program-specific restrictions separate, and use New York dates with complete requested months. The application requests one structured GLM-5.3-Flash generation through CAIL; no model repair or provider retry is made.
