...
The source text remains readable when the model service is unavailable. The published Pages configuration calls the canonical Railway backend at `https://guide-api-production-a1a1.up.railway.app`. That service holds credentials, accepts the `https://zmuhls.github.io` browser origin, and applies per-conversation and shared daily limits. With `CAIL_API_KEY` configured, each accepted message makes one GLM-5.3-Flash generation through the CAIL gateway. There are no automatic repair generations or provider fallbacks. Legacy Ollama/OpenRouter credentials are inactive while CAIL is configured. Warm-up refreshes calendar evidence without generating a chat answer.
