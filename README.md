...
The source text remains readable when the model service is unavailable. The published Pages configuration calls the canonical Railway backend at `https://guide-api-production-a1a1.up.railway.app`. That service holds credentials, accepts the `https://zmuhls.github.io` browser origin, and applies per-conversation and shared daily limits. With `CAIL_API_KEY` configured, each accepted message first makes one GLM-5.3-Flash generation through the CAIL gateway. If that live provider fails and OpenRouter is configured, the same grounded request receives one bounded GLM-5.3-Flash fallback generation. There are no automatic repair generations, canned answers, or classifier responses. Warm-up refreshes calendar evidence without generating a chat answer.
...
7. Every valid, non-private new request reaches GLM-5.3-Flash with five recent exchanges (ten messages) and approved page evidence. The structured-output schema includes the actual candidate IDs. The server validates shape and source ID without a semantic classifier or a second generation. Provider errors or invalid output remain operational failures, never fabricated Guide turns.
...
Conversation capture is off by default. With `FORTUNE_CONVERSATION_CAPTURE=none`, the server writes no query log and needs no chat database. Browser history is capped at five recent exchanges (ten messages) in tab-scoped session storage so it survives navigation between replica pages without being shared across tabs. Each new request receives those five prior exchanges; after it completes, only the oldest exchange is dropped. Open-ended questions sent to the active model must use public or invented information.
...
save. Draft saves never activate themselves or change the live guide. See the
